...
pAtom1 :: Parser Expr1
pAtom1 = (char '(' *> pExpr1 <* char ')') <|> Value1 <$> int 
...
pAtom2 :: Parser Expr2
pAtom2 = Value2 <$> int <|> (char '(' *> pExpr2 <* char ')')

pExpr2_mul :: Parser Expr2
pExpr2_mul = foldr1 Mul2 <$> sepBy1 pAtom2 (char '*')
...
--  Write a parser `pMatrix :: Parser [[Integer]]` for matrices of integers,
--   where rows of the matrix are separated by semicolons ';', and values in each
--   row of the matrix are separated by commas ','.
--  Each row of the matrix should be non-empty.
--

pMatrix :: Parser [[Int]]
pMatrix = sepBy1 (sepBy1 int (char ',') ) (char ';') 

tests :: [Bool]
tests = [ all (\(s,v) -> runParser pMatrix s == Just (v,"")) good
        , all (\s -> runParser pMatrix s == Nothing) bad
        ]
  where
    good :: [(String, [[Int]])]
    good =  [ ("10",      [[10]])
    ...
    bad :: [String]
    bad = [ ""          -- The matrix should not be empty.
          , ";1"        -- Each row of the matrix should be non-empty.
          , ","
          , "1,2,"
          , "1,3;"
          , "1,3;;10"
