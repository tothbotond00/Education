...
import elteCrussader.tile.Tile;
...
public class GameMap {
    ...
    public GameMap(){
        ...
        for (int i = 0; i < ROWS; i++){
            ...
            for (int j = 0; j < COLS; j++){
                switch (s.charAt(j)){
                    case 'M' -> this.tileMatrix[i][j] = new Tile('M',new Position(i,j));
                    case 'S' -> this.tileMatrix[i][j] = new Tile('S',new Position(i,j));
    ...
    public ArrayList<String> readMaps(){
        InputStream is;
        is = getClass().getResourceAsStream("/elteCrussader/res/Maps.txt");
